...
- **Kanban board** — Replaced the simple todo list with a three-column Kanban board (To Do, In Progress, Done) for a clear visual overview of task progress.
- **Drag-and-drop tasks** — Cards can be dragged and dropped between columns to update their status instantly.
...
- **CSV import with status support** — Importing a CSV reads the `kanban_board` column and places each task in the correct column automatically.
